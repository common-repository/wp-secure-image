<?php
	...
	$wpsiw_media_upload_nonce = sanitize_key($_GET['wpsiw_token']);

	$allow_uploads = false;

	if (!wp_verify_nonce( $wpsiw_media_upload_nonce, 'wpsiw_token'))
	{
		wp_nonce_ays("Error");
	}
	else 
	...
	$max_size = ($wpsiw_options["settings"]["max_size"]) ? $wpsiw_options["settings"]["max_size"] : 400 ;
		...
		<div id="tabs-1" class="wpsiw_addnew ui-tabs-panel ui-widget-content ui-corner-bottom">
			...
			<div class="wpsiw_upload_content">
				...
				<table> 
					<tr> 
						<td>
							<div class="mfu-wpsiw-plugin-uploader multiple">
        <input id="wpsiw-plugin-uploader-button" type="button" value="<?php esc_attr_e( 'Select Files' ); ?>" class="wpsiw-plugin-uploader-button button">
        <span class="ajaxnonce" id="<?php echo wp_create_nonce('wpsiw_upload_nonce'); ?>"></span>
							</div>
						</td>
						<tr>
						<td>
						<div id="wpsiw-upload-filename"></div>
						</td>
						</tr>
						<tr>
					<td>
					<div id="wpsiw-upload-status"></div>
					</td>
					</tr>
						<td> 
